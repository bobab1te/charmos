            ...
            {hidden.map((id) => (
              <li key={id}>
                <button
                  type="button"
                  onClick={() => show(id)}
                  className="flex w-full items-center justify-between rounded-xl px-3 py-2 text-left text-sm text-[var(--charm-ink)] transition duration-150 ease-out hover:bg-[var(--surface-interactive)] active:scale-[0.99]"
                >
                  {WIDGET_LABELS[id] ?? id}
