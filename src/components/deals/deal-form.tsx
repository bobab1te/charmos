...
export function DealForm({
  ...
  function setReferenceLink(index: number, value: string) {
    const next = values.referenceLinks.map((link, i) => (i === index ? value : link))
    set('referenceLinks', next)
  }

  function addReferenceLink() {
    set('referenceLinks', [...values.referenceLinks, ''])
  }

  function removeReferenceLink(index: number) {
    set(
      'referenceLinks',
      values.referenceLinks.length > 1 ? values.referenceLinks.filter((_, i) => i !== index) : values.referenceLinks,
    )
  }
      ...
      <Tabs defaultValue="details">
        ...
        <TabsContent value="content" className="mt-4 flex flex-col gap-4">
          ...
          <Section title="Reference Videos">
            <div className="flex flex-col gap-2">
              {values.referenceLinks.map((link, i) => (
                <div key={i} className="flex gap-2">
                  <Input
                    type="url"
                    value={link}
                    onChange={(e) => setReferenceLink(i, e.target.value)}
              ...
              <Button type="button" variant="outline" size="sm" onClick={addReferenceLink} className="w-fit gap-1.5">
                <Plus className="size-3.5" /> Add reference link
              </Button>
            </div>
          </Section>
        </TabsContent>
